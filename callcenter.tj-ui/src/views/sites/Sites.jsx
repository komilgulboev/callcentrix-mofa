...
import { sites as sitesApi, tenants as tenantsApi } from 'src/api'
...
export default function Sites() {
  ...
  const openEdit = (site) => {
    setEditing(site)
    setForm({ names: { ...EMPTY_NAMES, ...site.names }, active: site.active ?? true })
    setModal(true)
  }
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      if (editing) await sitesApi.update(tenantId, editing.id, { names: form.names, active: form.active })
      else         await sitesApi.create(tenantId, { names: form.names, active: form.active })
      setModal(false)
      load()
    } catch (e) { setError(e.message) }
  ...
  const handleDelete = async (id) => {
    ...
    try { await sitesApi.remove(tenantId, id); load() }
    catch (e) { setError(e.message) }
  }
  ...
  return (
    <>
      <div className="d-flex align-items-center justify-content-between mb-4">
        <h4 className="mb-0">{t('sites.title')}</h4>
        <div className="d-flex gap-2 align-items-center">
          ...
          <CButton color="primary" onClick={openCreate} disabled={!tenantId}>
            <CIcon icon={cilPlus} className="me-2" />{t('sites.new_site')}
          </CButton>
        </div>
      </div>
      ...
      {superAdmin && (
        <div className="mb-3" style={{ maxWidth: 340 }}>
          ...
          <CFormSelect value={selectedTenant} onChange={(e) => setSelectedTenant(e.target.value)}>
            <option value="">{t('sites.select_tenant')}</option>
            ...
            <CTable hover responsive>
              <CTableHead>
                <CTableRow>
                  ...
                  <CTableHeaderCell>{t('sites.col_name')} ({LANG_LABEL[displayLang]})</CTableHeaderCell>
                  <CTableHeaderCell>RU</CTableHeaderCell>
                  <CTableHeaderCell>TJ</CTableHeaderCell>
                  <CTableHeaderCell>EN</CTableHeaderCell>
                  <CTableHeaderCell>{t('sites.col_status')}</CTableHeaderCell>
              ...
              <CTableBody>
                {rows.map((s) => (
                  <CTableRow key={s.id}>
                    <CTableDataCell className="text-muted">{s.id}</CTableDataCell>
                    <CTableDataCell className="fw-semibold">{siteName(s, displayLang)}</CTableDataCell>
                    <CTableDataCell className="text-muted small">{s.names?.ru || '—'}</CTableDataCell>
                    <CTableDataCell className="text-muted small">{s.names?.tj || '—'}</CTableDataCell>
                    <CTableDataCell className="text-muted small">{s.names?.en || '—'}</CTableDataCell>
                    <CTableDataCell>
                      <CBadge color={s.active ? 'success' : 'secondary'}>
                        {s.active ? t('sites.active') : t('sites.inactive')}
                      </CBadge>
                    </CTableDataCell>
                    <CTableDataCell className="text-end">
                      <div className="d-flex gap-1 justify-content-end">
                        <CButton size="sm" color="light" onClick={() => openEdit(s)}>
                          <CIcon icon={cilPencil} />
                        </CButton>
                        <CButton size="sm" color="danger" onClick={() => handleDelete(s.id)}>
                          <CIcon icon={cilTrash} />
                        </CButton>
                      </div>
                    </CTableDataCell>
                ...
                {!rows.length && (
                  <CTableRow>
                    <CTableDataCell colSpan={7} className="text-center text-muted py-4">
                      {tenantId ? t('sites.empty') : t('sites.empty_select')}
                    </CTableDataCell>
                  </CTableRow>
                )}
              </CTableBody>
      ...
      <CModal visible={modal} onClose={() => setModal(false)}>
        <CModalHeader>
          <CModalTitle>{editing ? t('sites.edit') : t('sites.new_site')}</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CForm className="d-flex flex-column gap-3">
            {LANGS.map((l) => (
              <div key={l}>
                <CFormLabel>{t('sites.name_in_language', { lang: t(`header.lang_${l}`) })}</CFormLabel>
                <CFormInput
                  value={form.names[l]}
                  onChange={(e) => setName(l, e.target.value)}
                  placeholder={t('sites.placeholder_in_language', { lang: t(`header.lang_${l}`) })}
                />
              </div>
            ))}
            <CFormCheck
              label={t('sites.active_label')}
              checked={form.active}
