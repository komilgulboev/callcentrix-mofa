...
type Config struct {
	...
	JWTSecret  string
	JWTMinutes int
	// JWTOperatorMinutes is the token TTL for Operator accounts (user_type=3),
	// which need a longer-lived session than other roles (a full 12h shift).
...
func Load() *Config {
	...
	operatorTTL, _ := strconv.Atoi(getEnv("JWT_OPERATOR_TTL_MINUTES", "720")) // 12h
	...
	return &Config{
		...
		JWTMinutes:         ttl,
		JWTOperatorMinutes: operatorTTL,
